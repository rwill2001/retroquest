...
/**
 * 8-bit waterfall cave exit animation.
 *
 * Player emerges from the moss-ringed cave mouth behind a parted waterfall,
 * walks forward toward the camera (growing larger), while the waterfall closes
 * back together and the cliff recedes into a forest backdrop.
 *
 * Sequence (total ~3.0 s)
 *   0–12%  : zoomed-in cliff + cave mouth + parted waterfall; interior glow fades
 *  10–45%  : player emerges small, walks toward camera (grows)
 *  25–60%  : waterfall closes back together, covering cave mouth
 *  40–80%  : cliff recedes into distance; forest canopy and sky open up
 *  80–100% : fade to overworld
 */
public class WaterfallCaveExitAnimation implements AnimationGuard.Cancellable {
    ...
    public WaterfallCaveExitAnimation(Retroquest game, String townName) {
        this.game     = game;
    ...
    public void start() {
        ...
        java.util.Random rng = new java.util.Random(townName.hashCode() ^ 0xCAFE);
        fired     = false;
        paintFailed = false;
    ...
    private void doPaint(Graphics2D g, int W, int H) {
        ...
        // Cliff recedes as player walks away from it — mirror of entry
        float cliffScale = 1f - Math.min(1f, Math.max(0f, (p - 0.40f) / 0.45f)) * 0.78f;
        int cliffCX = W / 2;
        int cliffTop = skyH - (int)(60 * cliffScale);
        int cliffH   = (int)(H * 0.82f * cliffScale);
        int cliffW   = (int)(W * 0.80f * cliffScale);

        // Waterfall is fully parted at start, closes back together
        float closeP = 1f - Math.min(1f, Math.max(0f, (p - 0.20f) / 0.40f));
        paintCliffFace(g, cliffCX, cliffTop, cliffW, cliffH, cliffScale, closeP, now);

        paintFerns(g, W, H);
        paintMist(g, W, H);

        // Player exits cave: grows as walks forward
        float walkP = Math.min(1f, Math.max(0f, (p - 0.08f) / 0.72f));
        if (walkP > 0) paintPlayerExiting(g, W, H, walkP, now);

        // Warm glow inside cave mouth that fades as player emerges
        float glowFade = 1f - Math.min(1f, p / 0.22f);
        if (glowFade > 0) {
            int glowR = (int)(cliffW * 0.10f);
            int gcx   = cliffCX;
            int gcy   = cliffTop + cliffH / 2;
        ...
        float nameP = Math.min(1f, Math.max(0f, (p - 0.30f) / 0.20f))
                    * (1f - Math.min(1f, Math.max(0f, (p - 0.72f) / 0.12f)));
        if (nameP > 0) paintFarewell(g, W, H, nameP);

        // Gentle fade to overworld
        float fadeOut = Math.min(1f, Math.max(0f, (p - 0.82f) / 0.18f));
        if (fadeOut > 0) {
    ...
    private void paintCliffFace(Graphics2D g, int cx, int top, int w, int h,
                                float scale, float closeP, long now) {
        int x = cx - w / 2;
        ...
        // partP = 1 when waterfall fully parted (start of exit), drops to 0 as it closes
        float partP = closeP;
    ...
    private void paintPlayerExiting(Graphics2D g, int W, int H, float walkP, long now) {
        // Opposite of entry: start small and far (behind falls), grow as walks forward
        float yT      = 0.46f + walkP * 0.28f;
        float sizeT   = 0.15f + walkP * 0.85f;
        float fadeIn  = Math.min(1f, walkP / 0.15f);
        int px = W / 2;
        int py = (int)(yT * H);
        ...
        g.setColor(new Color(0, 0, 0, (int)(60 * fadeIn)));
    ...
    private void paintFarewell(Graphics2D g, int W, int H, float alpha) {
        int a = (int)(alpha * 230);
        String farewell = "Back into the open air...";

        g.setFont(F_SUB);
        FontMetrics fmS = g.getFontMetrics();
        int wy = (int)(H * 0.88f);
        int wx = (W - fmS.stringWidth(farewell)) / 2;
        g.setColor(new Color(0, 0, 0, a / 2));
        g.drawString(farewell, wx + 2, wy + 2);
        g.setColor(new Color(200, 230, 240, a));
        g.drawString(farewell, wx, wy);
    }
}
