...
/**
 * Full-screen abyssal depths overlay with three water-themed mini-games:
 * Pressure Diving, Bone Racing, and Deep Sea Fishing. Follows the same
 * pattern as {@link CasinoOverlay} and {@link ForgeArenaOverlay}.
 */
public class AbyssalDepthsOverlay {

    // ── Deep ocean palette ──────────────────────────────────────────────────
    static final Color PANEL_BG     = new Color(  4,  10,  20);
    static final Color ABYSS_TEAL   = new Color(  0, 180, 180);
    static final Color BIOLUM       = new Color( 80, 255, 160);
    static final Color CYAN_GLOW    = new Color(  0, 200, 255);
    static final Color PRESSURE_RED = new Color(200,  60,  60);
    ...
    private static final String[] MENU_ITEMS = {"Pressure Diving", "Bone Racing", "Deep Sea Fishing", "Leave"};
    ...
    // Mini-game instances
    private final PressureDivingGame divingGame;
    private final BoneRacingGame racingGame;
    private final DeepSeaFishingGame fishingGame;
    ...
    public AbyssalDepthsOverlay(Retroquest game) {
        this.game = game;
        for (int i = 0; i < menuRects.length; i++) menuRects[i] = new Rectangle();

        divingGame  = new PressureDivingGame(game, this, rng);
        racingGame  = new BoneRacingGame(game, this, rng);
        fishingGame = new DeepSeaFishingGame(game, this, rng);
    }
    ...
    private void paintMenu(Graphics2D g, int px, int py, int pw, int ph) {
        ...
        g.setFont(F_TITLE);
        g.setColor(ABYSS_TEAL);
        String title = "THE ABYSSAL DEPTHS";
        ...
        String sub = game.getPlayer().isGamblingCapped()
            ...
            : "Wager your gold in the crushing deep...";
