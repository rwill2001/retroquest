...
/**
 * Full-screen casino overlay with four mini-games: Game of Chance (roulette-style
 * wheel), Blackjack, Wizard's Dice (craps), and Dragon's Darts. All graphics are
 * 8-bit pixel art rendered via {@code Graphics2D.fillRect()} — no external images.
 *
 * <p>Follows the standard overlay contract: {@link #paint(Graphics2D, int, int)}
 * called from {@link GamePanel}, input via {@link #handleKey(KeyEvent)} and
 * {@link #handleClick(int, int)}.
 *
 * <p>Each mini-game is implemented as a separate class implementing {@link MiniGame}.
 * This class acts as a thin dispatcher: it owns the menu, betting state, and entry
 * animation, delegating game-specific logic and rendering to the active mini-game.
 */
public class CasinoOverlay {

    // ── Palette (custom PANEL_BG — slightly different from standard) ───────────
    static final Color PANEL_BG = new Color( 14,  18,  28);
    static final Color GOLD_COL = new Color(255, 215,   0);
    ...
    private static final String[] MENU_ITEMS = {"Game of Chance", "Blackjack", "Wizard's Dice", "Dragon's Darts", "Leave"};
    ...
    // The four mini-game instances
    private final GameOfChanceGame gocGame;
    private final BlackjackGame bjGame;
    private final WizardsDiceGame wdGame;
    private final DragonsDartsGame ddGame;

    // Geometry cache for mouse hit-testing
    private final Rectangle[] menuRects = new Rectangle[5];
    ...
    public CasinoOverlay(Retroquest game) {
        this.game = game;
        for (int i = 0; i < menuRects.length; i++) menuRects[i] = new Rectangle();

        gocGame = new GameOfChanceGame(game, this, rng);
        bjGame  = new BlackjackGame(game, this, rng);
        wdGame  = new WizardsDiceGame(game, this, rng);
        ddGame  = new DragonsDartsGame(game, this, rng);
    }
    ...
    public void handleKey(KeyEvent e) {
        ...
        if (currentGame != null) {
            if (currentGame.isShowingResult()) {
                // Any key on a final result screen returns to menu
                currentGame.handleKey(e); // let the game handle cleanup
            } else {
    ...
    private void confirmMenu() {
        switch (menuSelection) {
            ...
            case 1 -> { // Blackjack
                ...
                betAmount = Math.min(lastBet, Math.min(game.getPlayer().getGold(), game.getPlayer().getMaxBet()));
                if (betAmount < 1) betAmount = 1;
                bjGame.reset(betAmount);
                currentGame = bjGame;
                inMenu = false;
            }
            case 2 -> { // Wizard's Dice
                if (!checkGold() || !checkGamblingCap()) return;
                betAmount = Math.min(lastBet, Math.min(game.getPlayer().getGold(), game.getPlayer().getMaxBet()));
                if (betAmount < 1) betAmount = 1;
                wdGame.reset(betAmount);
                currentGame = wdGame;
                inMenu = false;
            }
            case 3 -> { // Dragon's Darts
                if (!checkGold() || !checkGamblingCap()) return;
                betAmount = Math.min(lastBet, Math.min(game.getPlayer().getGold(), game.getPlayer().getMaxBet()));
                if (betAmount < 1) betAmount = 1;
                ddGame.reset(betAmount);
                currentGame = ddGame;
                inMenu = false;
            }
            case 4 -> close(); // Leave
    ...
    /** Called by mini-games to return to the menu. */
    void backToMenu() {
        currentGame = null;
        inMenu = true;
    }

    // ── Bet adjustment (called by mini-games) ───────────────────────────────────
