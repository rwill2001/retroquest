...
/**
 * Full-screen memory games overlay with three shadow-themed mini-games:
 * Shadow Veil, Echoes, and Hollow Riddles.
 * Hosted in Dusthaven on Island 6 (Umbryn / The Shadow Trenches).
 */
public class MemoryGamesOverlay {

    // ── Shadow palette ────────────────────────────────────────────────────────
    static final Color PANEL_BG      = new Color( 12,   8,  20);
    static final Color SHADOW_PURPLE = new Color(100,  70, 160);
    static final Color GHOST_WHITE   = new Color(210, 210, 230);
    static final Color VOID_BLUE     = new Color( 50,  60, 110);
    static final Color GRIEF_GREY    = new Color(130, 120, 140);
    ...
    private static final String[] MENU_ITEMS = {
        "Shadow Veil", "Echoes", "Hollow Riddles", "Leave"
    };
    ...
    private final ShadowVeilGame    veilGame;
    private final EchoesGame        echoGame;
    private final HollowRiddlesGame riddleGame;
    ...
    public MemoryGamesOverlay(Retroquest game) {
        this.game = game;
        for (int i = 0; i < menuRects.length; i++) menuRects[i] = new Rectangle();

        veilGame   = new ShadowVeilGame(game, this, rng);
        echoGame   = new EchoesGame(game, this, rng);
        riddleGame = new HollowRiddlesGame(game, this, rng);
    }
    ...
    private void paintMenu(Graphics2D g, int px, int py, int pw, int ph) {
        ...
        String sub = game.getPlayer().isGamblingCapped()
            ...
            : "What the shadows remember, can you?";
        ...
        for (int i = 0; i < MENU_ITEMS.length; i++) {
            ...
            if (i == menuSelection) {
                g.setColor(new Color(15, 8, 35)); g.fillRect(ix, iy, iw, itemH - 4);
                g.setColor(SHADOW_PURPLE); g.drawRect(ix, iy, iw, itemH - 4);
                g.setColor(GHOST_WHITE);
            } else {
                g.setColor(new Color(14, 10, 24)); g.fillRect(ix, iy, iw, itemH - 4);
