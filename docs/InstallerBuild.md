...
- The game fat JAR (`retroquest-0.9.0-fat.jar`)
...
Produces `target/retroquest-0.9.0-fat.jar`.
...
`jdeps --print-module-deps --ignore-missing-deps target/retroquest-0.9.0-fat.jar`.
...
| `target/retroquest-0.9.0-fat.jar`| `installer/`                 |
...
```
target/
  retroquest-0.9.0.jar          # thin JAR (not used for distribution)
  retroquest-0.9.0-fat.jar      # game fat JAR (bundled into installer)
  retroquest-installer.jar             # distributable installer  <-- ship this
  jre/                                 # bundled JRE (bundled into installer)
  dependency-reduced-pom.xml           # generated by shade plugin (safe to ignore)
```
...
**Assembly fails: `retroquest-0.9.0-fat.jar` not found**
